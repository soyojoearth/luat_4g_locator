...
#define PRODUCT_KEY "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" // 产品key
...
void lbsloc_request_task(void *param)
{
    while (1)
    {
        ...
        if(dpValue_hasSatellite){//有卫星信号，就不需要LBS定位
            LUAT_DEBUG_PRINT("hasSatellite True, No Need LBS");
        }
        else{
            LUAT_DEBUG_PRINT("hasSatellite False, Need LBS");
